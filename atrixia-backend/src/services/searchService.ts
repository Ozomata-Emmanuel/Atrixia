...
export const getSearchById = async (userId: string, searchId: string) => {
  ...
  if (searchResult[0].userId !== userId) {
    throw new AppError('Search belongs to different user', 403);
  }

  return searchResult[0];
};

export const getSearchHistory = async (userId: string, limit: number = 10, offset: number = 0) => {
  ...
  // We can also get total count if needed for pagination
