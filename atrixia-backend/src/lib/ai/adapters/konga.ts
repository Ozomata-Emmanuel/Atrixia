      ...
      try {
        ...
        const mapped = items
          ...
          .map((item: any) => {
            ...
            const imageRaw = item.image || item.thumbnail;
            const image = typeof imageRaw === 'string'
              ? imageRaw
              : imageRaw?.url || imageRaw?.src || null;
            ...
            return {
              ...
              image,
              productUrl,
              seller: item.seller_name || item.store_name || 'Konga Seller',
              sellerRating: item.rating ? Math.round((parseFloat(item.rating) / 5) * 100) : null,
              reviewCount: parseInt(String(item.review_count || item.reviews_count || '0'), 10),
              shippingCost: 0,
              shippingEstimate: 'Standard Delivery (1-3 days)',
              availability: item.is_in_stock !== false,
              condition: 'new' as const,
              category: options?.category || item.category_name || 'General',
              attributes: {},
              confidence: 80,
              rawData: {},
              description: desc,
